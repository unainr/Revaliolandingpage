...
import { ArrowRight, CheckCircle, Shield, Clock, Building } from "lucide-react";
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-10">
            ...
            {/* Achievements Grid */}
            <div 
              className={`grid grid-cols-1 sm:grid-cols-2 gap-6 transition-all duration-500  ease-in ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            >
              {achievements.map((achievement, index) => {
                const isActive = isMobile || activeAchievement === index;

                return (
                  <div 
                    key={index} 
                    className="flex flex-col space-y-3 p-4 rounded-xl transition-all duration-500 hover:bg-white dark:hover:bg-gray-800 hover:shadow-lg border border-transparent hover:border-gray-100 dark:hover:border-gray-700"
                    onMouseEnter={!isMobile ? () => setActiveAchievement(index) : undefined}
                    onMouseLeave={!isMobile ? () => setActiveAchievement(null) : undefined}
                  >
                    <div className="flex items-center space-x-3">
                      <div 
                        className="h-10 w-10 rounded-lg flex items-center justify-center transition-all duration-500 ease-in"
                        style={{ 
                          backgroundColor: isActive ? achievement.color : `${achievement.color}20`,
                          transform: isActive ? 'scale(1.1)' : 'scale(1)'
                        }}
                      >
                        {React.createElement(achievement.icon, { 
                          className: "h-5 w-5 transition-all duration-300",
                          style: { color: isActive ? 'white' : achievement.color }
                        })}
                      </div>
                      <span 
                        className="font-medium transition-all duration-300"
                        style={{ color: isActive ? achievement.color : 'inherit' }}
                      >
                        {achievement.text}
                      </span>
                    </div>
                    ...
                    <div 
                      className="text-sm text-muted-foreground pl-12 transition-all duration-500 ease-in"
                      style={{ 
