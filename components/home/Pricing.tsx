...
const PricingComponent = () => {
  ...
  return (
     <section className="relative py-20 overflow-hidden bg-white dark:bg-gray-900 transition-colors duration-200">
      ...
      <div className={`absolute top-40 -left-20 w-72 h-72 rounded-full bg-gradient-to-r from-purple-300/30 to-blue-300/30 dark:from-purple-600/20 dark:to-blue-600/20 blur-3xl ${isLoaded ? 'animate-glow' : ''}`}></div>
      <div className={`absolute bottom-20 right-10 w-80 h-80 rounded-full bg-gradient-to-r from-pink-300/30 to-orange-200/30 dark:from-pink-600/20 dark:to-orange-500/20 blur-3xl ${isLoaded ? 'animate-glow-delayed' : ''}`}></div>
          ...
          <div className={`text-center space-y-4 mb-16 ${isLoaded ? 'animate-reveal' : 'opacity-0'}`}>
            ...
            <div className="flex items-center justify-center mt-8 space-x-4">
              ...
              >
                ...
                <span
                  aria-hidden="true"
                  className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white dark:bg-gray-100 shadow ring-0 transition duration-200 ease-in-out ${isYearly ? 'translate-x-6' : 'translate-x-0'}`}
                />
              </button>
              <span className={`text-sm font-medium flex items-center ${isYearly ? 'text-gray-900 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400'}`}>
                Yearly
            ...
            {tiers.map((tier, index) => (
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <div className="mt-6">
                    <span className="text-5xl font-bold text-gray-900 dark:text-gray-100">${tier.price}</span>
                    <span className="text-base text-gray-500 dark:text-gray-400">/{isYearly ? 'year' : 'month'}</span>
                  </div>
                </div>
